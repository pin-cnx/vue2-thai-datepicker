...
describe('Locale', () => {
  it('render the correct default locale', () => {
    ...
    expect(wrapper.find('.mx-table-date td').element.title).toBe('2562-09-30');
  });
  ...
  it('prop: lang - string', async () => {
    ...
    expect(wrapper.find('.mx-table-date .active').element.title).toBe('Oct 10, 2562');
    ...
    expect(wrapper.find('.mx-table-date th').text()).toBe('一');
    expect(wrapper.find('.mx-table-date .active').element.title).toBe('10月 10, 2562');
